...
// IANA zone -> POSIX TZ, for resolving a typed city name / browser-geolocation point online: the
// Open-Meteo geocoding + forecast APIs report an IANA zone ("Europe/Paris"), but configTzTime()
// wants a POSIX TZ. A hit here gives DST-correct time; a miss falls back to a fixed UTC offset.
struct RegionTZ { const char* iana; const char* posix; };
static const RegionTZ REGION_TZMAP[] = {
...
static const int REGION_NTZMAP = sizeof(REGION_TZMAP) / sizeof(REGION_TZMAP[0]);
